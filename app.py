...
from model.hepsiburda_datas import db
...
if __name__ == "__main__":
    ...
    #notebook_scheduler_thread = threading.Thread(target=lambda: asyncio.run(schedule_task_for_vatan(s, app, 'postgresql://postgres:test@localhost:5432/FinalProject')))  # Use asyncio.run to run the async function
    notebook_scheduler_thread = threading.Thread(target=lambda: asyncio.run(schedule_task_for_vatan(app, 'postgresql://postgres:test@localhost:5432/FinalProject')))  # Use asyncio.run to run the async function
